...
function getMapFromBase64(url: string, base: string): SourceMap | null {
...
export function getMapFromUrl ( url: string, base: string ): Promise<SourceMap | null> {
...
export function getMapFromUrlSync ( url: string, base: string ): SourceMap | null {
    const map = getMapFromBase64(url, base);
    if (map) {
        return map;
    }
